...
var DbConnector = (function () {
	...
	obj.prototype.getNextReferencePoint = function (excludes, callback) {
		this.client.query(
			'SELECT id, name, ST_AsGeoJSON(the_geom) AS geom FROM foursq_venues ' + ((excludes.length > 0) ? 'WHERE id NOT IN (' + excludes.join(', ') + ') ' : '') + 'LIMIT 1',
	 ...
	 */
	 ...
	 obj.prototype.getMatchingCandidates = function(point, callback) {
	 	var geometryWkt = GEOJSON2WKT.convert(point);
	 	...
	 	this.client.query(
	 		'SELECT id, name, ST_AsGeoJSON(the_geom) AS geom FROM facebook WHERE ST_Intersects(the_geom, ST_Buffer(ST_Geomfromtext(\'' + geometryWkt + '\', 4326), 0.05))',
	 		function (error, result) {
	 			if (error) throw new Error('Error while reading facebook venues from database: \n' + JSON.stringify(error));
	 			else matchingCandidates.facebook = result.rows;
	 			pending--;
	 			if (pending === 0) callback(matchingCandidates);
	 		}
	 	);

	 	this.client.query(
	 		'SELECT osm_id as id, name, ST_AsGeoJSON(way) AS geom FROM planet_osm_point WHERE name IS NOT null AND ST_Intersects(way, ST_Buffer(ST_Geomfromtext(\'' + geometryWkt + '\', 4326), 0.05))',
	 		function (error, result) {
